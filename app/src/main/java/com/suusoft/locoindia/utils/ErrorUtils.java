...
public class ErrorUtils {
    ...
    public static Snackbar showSnackbar(View view, @StringRes int errorMessage, Exception e,
        ...
        if (noConnection || timeout) {
            return showSnackbar(view, R.string.msg_no_network, actionLabel, clickListener);
        } else if (errorMessage == 0) {
            return showSnackbar(view, error, actionLabel, clickListener);
        } else if (error.equals("")) {
            return showSnackbar(view, errorMessage, AppController.getInstance().getString(R.string.msg_no_network), actionLabel, clickListener);
        } else {
    ...
    public static void showErrorToast(QBResponseException exception) {
        Toast.makeText(AppController.getInstance(), String.format("[ERROR] Request has been completed with errors: %s", exception.getErrors()
                + ", code: " + exception.getHttpStatusCode()), Toast.LENGTH_SHORT).show();
    }
